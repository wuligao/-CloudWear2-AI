...
- CloudWear H5 穿搭生成接口位于 `vivy-modules/vivy-system/src/modules/outfit`，对外提供 `/api/generate-outfit` 任务创建、查询和 SSE 状态流。
...
穿搭生成使用兼容 OpenAI API 的文本和图片服务。可在启动 `vivy-system` 前设置环境变量，或复制 `vivy-modules/vivy-system/src/config/config.local.yaml.example` 为 `config.local.yaml` 后填写：

```bash
OPENAI_TEXT_API_KEY=
OPENAI_TEXT_BASE_URL=http://your-text-provider/v1
